window.digiAppSkinning = {
    colors: {
        primary: '#ffb700'
    },
    slider: {
        isShadowExist: true,
        sliderShadowColor: 'linear-gradient(to right, rgba(26,26,26,0) 0%, rgba(20,19,19,1) 100%)',
        sliderInlineShadowColor: 'linear-gradient(to right, rgba(26,26,26,0) 0%, rgba(26,26,26,1) 100%)'
    },
    orderInfo: {
        orderInfoBackground: '#262626',
        orderInfoColor: '#cccccc',
        orderInfoRightColor: '#ffffff'
    },
    modal: {
        dialogBgColor: 'rgba(64, 64, 64, 0.6)',
        dialogBoxShadow: '0 3px 6px rgba(0, 0, 0, 0.3)'
    }
};
